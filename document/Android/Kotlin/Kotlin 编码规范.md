...
这里两个 `Platform.kt` 文件都在同一个包中，因此 Kotlin JVM 编译器生成两个文件门面，它们都有 FQN `myPackage.PlatformKt`。 这就会产生 "Duplicate JVM classes"（"重复的 JVM 类"）错误。
...
如果使用一个 private 或者 internal 的类型别名来避免名称冲突，请优先使用 `import ...... as ......`。
